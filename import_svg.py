...
#### Common utilities ####
# FIXME load the id as the name of the curves
SVGEmptyStyles = {'useFill': None,
                  'fill': None}


def SVGCreateCurve(context, group='collection'):
    """
...
def SVGGetMaterial(color, context, mat_name='SVGMat'):
    """
...
def SVGParseStyles(node, context, svg_filepath):
    """
    ...
    class_style = node.getAttribute('class')
    if class_style:
        mat_name = f".{class_style}"
        styles['useFill'] = True if mat_name != "." else False
        styles['fill'] = bpy.data.materials[mat_name]
        return styles
...
def id_names_from_node(node, ob):
    # print(f"{node}, {node.getAttribute('id')}")
...
class SVGGeometry:
    ...
    __slots__ = ('_node',  # XML node for geometry
                 ...
                 '_creating',
                 '_svg_filepath')  # Flag if geometry is already creating
    # for this node
    # need to detect cycles for USE node

    def __init__(self, node, context, svg_filepath):
        """
        ...
        self._svg_filepath = svg_filepath
...
def create_material(node, context):
    print(node)
    style_content = node.firstChild.nodeValue.strip()

    # Split the style content into individual style definitions
    style_list = style_content.split('}')

    # Dictionary to hold class names and their properties
    styles_dict = {}

    # Process each style definition
    for style in style_list:
        if '{' in style:
            class_names, properties = style.split('{')
            class_names = class_names.strip().split(', ')
            properties = properties.strip()

            # Add properties to each class name in the dictionary
            for class_name in class_names:
                if class_name not in styles_dict:
                    styles_dict[class_name] = []
                styles_dict[class_name].append(properties)

    # Print the dictionary with class names and their properties
    for class_name, properties in styles_dict.items():
        print(f"{class_name}:")
            ...
            if ";" in prop:
                ...
                for v in varst:
                    if "fill:" in v:
                        fill = v.replace("fill: ","")
                        print(f"{fill}")
                        SVGGetMaterial(fill, context, class_name)
            else: 
                if "fill:" in prop:
                    fill = prop.replace("fill: ","")
                    print(f"{fill}")
                    SVGGetMaterial(fill, context, class_name)
...
class SVGGeometryContainer(SVGGeometry):
    ...
    __slots__ = ('_geometries',  # List of chold geometries
                 ...
                 'svg_filepath')  # svg_filepath, used for saving path
    ...
    def __init__(self, node, context, svg_filepath):
        ...
        super().__init__(node, context, svg_filepath)
    ...
    def parse(self):
        ...
        if type(self._node) is xml.dom.minidom.Element:
            self._styles = SVGParseStyles(self._node, self._context, self._svg_filepath)
        ...
        for node in self._node.childNodes:
            ...
            if name == 'style' :
                create_material(node, self._context)
                continue    
            if name == 'path' :
                continue     
            ob = parseAbstractNode(node, self._context, self._svg_filepath)
            if ob is not None:
...
class SVGGeometryPATH(SVGGeometry):
    ...
    def __init__(self, node, context, svg_filepath, group=""):
        """
        Initialize SVG path
        """
    ...
    def parse(self, group=""):
        ...
        style = self._node.getAttribute('class')
        self._styles = SVGParseStyles(self._node, self._context, self._svg_filepath)
    ...
    def _doCreateGeom(self, instancing):
        ...
        """
        if not self._group:
            svg_name = os.path.basename(self._svg_filepath)
            ob = SVGCreateCurve(self._context, svg_name)
        else:
            ob = SVGCreateCurve(self._context, self._group)
        cu = ob.data
...
class SVGGeometrySYMBOL(SVGGeometryContainer):
    ...
    def _doCreateGeom(self, instancing, svg_filepath):
        """
        Create real geometries
        """
...
class SVGGeometryG(SVGGeometryContainer):
    ...
    def __init__(self, node,  context, svg_filepath):
        ...
        svg_name = os.path.basename(self._svg_filepath)
        id_g = self._node.getAttribute("id")
        if id_g and svg_name:
            # print(id_g)
            parent = bpy.data.collections[svg_name]
            collection = bpy.data.collections.new(name=id_g)
            parent.children.link(collection)
        for path in node.getElementsByTagName('path'):
            name = path.tagName.lower()
            geomClass = svgGeometryClasses.get(name)
            ob = geomClass(path, context, svg_filepath)
            if "path" == name:
                ob.parse(id_g)
            self._geometries.append(ob)
...
class SVGGeometrySVG(SVGGeometryContainer):
    ...
    def _doCreateGeom(self, instancing):
        ...
        if len(viewbox) == 4 and unit in ('cm', 'mm', 'in', 'pt', 'pc'):
            ...
            matrix = matrix @ Matrix.Scale(unitscale,
        ...
        if self._node.getAttribute('viewBox'):
            viewbox = parse_array_of_floats(self._node.getAttribute('viewBox'))
            matrix = matrix @ matrix.Translation(
                [0.0, - viewbox[1] - viewbox[3], 0.0])
...
class SVGLoader(SVGGeometryContainer):
    ...
    def __init__(self, context, filepath, do_colormanage):
        ...
        m = Matrix()
        m = m @ Matrix.Scale(1.0 / 90.0 * 0.3048 / 12.0,
                             4, Vector((1.0, 0.0, 0.0)))
        m = m @ Matrix.Scale(-1.0 / 90.0 * 0.3048 / 12.0,
                             4, Vector((0.0, 1.0, 0.0)))
        ...
        super().__init__(node, self._context, svg_filepath)
...
def parseAbstractNode(node, context, svg_filepath):
    name = node.tagName.lower()
    ...
    if geomClass is not None:
        ob = geomClass(node, context, svg_filepath)
        ob.parse()
...
def get_parent_collection(child_name):
    def find_parent(collection, target_name):
        for col in collection.children:
            if col.name == target_name:
                return collection
            parent_col = find_parent(col, target_name)
            if parent_col:
                return parent_col
        return None

    for col in bpy.context.scene.collection.children:
        parent_col = find_parent(col, child_name)
        if parent_col:
            return parent_col
    return None

def rearrange_coll(svg_name):
    print("REARRANGE")
    collections_to_move = []
    collections_to_store = []

    all_collections = bpy.data.collections[svg_name].children

    for col in all_collections:
        without_p_val = col.name.split("|")[1]
        if len(without_p_val.split("_")) > 1:
            collections_to_move.append(col.name)
        else:
            collections_to_store.append(col.name)
            
    # print("Move", collections_to_move)        
    # print("Store", collections_to_store)        
    
    for move in collections_to_move:
        without_p_val = move.split("|")
        names = without_p_val[1].split("_")
        
        if len(names) > 1:
            root_name = "|".join((without_p_val[0], names[0]))
            # print(root_name, root_name in collections_to_store)
            if root_name in collections_to_store:
                try:
                    col_ = bpy.data.collections[move]
                    # print(col_, "<<<<<", root_name)
                    get_parent_collection(move).children.unlink(col_)
    #                
                    root = all_collections[root_name]
                    root.children.link(col_)
                except:
                    print("fail or already moved out", move)

def batch_rename(filepath):
    
    # svg_name = "igcc_master_styleS_buff_female.svg"
    svg_name = os.path.basename(filepath)
    all_col = bpy.data.collections[svg_name].children_recursive
    split_name = svg_name.split(".")[0].replace("igcc_master_styleS_","")
    for col in all_col:
        c_name = col.name
        if c_name != svg_name and "igcc_master_styleS" not in c_name:
            if "|" not in c_name:
                col.name = "|".join((split_name,c_name))
    rearrange_coll(svg_name)
    bulk_convert_to_gpencils(svg_name)
...
def load(operator, context, filepath=""):
    ...
    try:
        ...
        operator.report({'WARNING'}, tip_("Unable to parse XML, %s:%s for file %r") % (
            type(e).__name__, e, filepath))
        return {'CANCELLED'}
...
def bulk_convert_to_gpencils(svg_name):
    curves_ = bpy.data.collections[svg_name].children_recursive
    for cur in curves_:
        objects_col = cur.objects
        if objects_col:
            for obj in objects_col:
                print(obj)
                convert_curve_to_grease_pencil_and_maintain_collection(obj)
            for obj in objects_col:
                obj.select_set(True)
                bpy.context.view_layer.objects.active = obj
            bpy.ops.object.join()
            act = bpy.context.active_object
            for idx, layer in enumerate(act.data.layers):
                try:
                    layer.info = f"layer_{idx}"
                except:
                    layer.name = f"layer_{idx}"
            original_collections = act.users_collection[:]
            get_init_name = original_collections[0].name.split("|")
            for coll in original_collections:
                if bpy.data.objects.get(f"{get_init_name[1].split('_')[0]}"):
                    act.name = f"{get_init_name[0]}|{get_init_name[1].split('_')[0]}"
                else:
                    act.name = f"{get_init_name[1].split('_')[0]}"
                    

def convert_curve_to_grease_pencil_and_maintain_collection(curve_obj):
    # Ensure the object is a curve
    
    name_c = curve_obj.name
    # Store the collections the curve belongs to
    original_collections = curve_obj.users_collection[:]
    ...
    # Convert the curve to grease pencil
    if curve_obj.type == 'CURVE':
        print("The selected object is a curve.")
        try:
            bpy.ops.object.convert(target='GPENCIL')
        except:
            bpy.ops.object.convert(target='GREASEPENCIL')

    # Get the new grease pencil object
    gpencil_obj = bpy.context.active_object
    
    # Ensure the grease pencil object is linked to the original collections
    for coll in original_collections:
        if gpencil_obj.name not in coll.objects:
            coll.objects.link(gpencil_obj)
    
    # Optionally, remove the grease pencil object from other collections
    for coll in gpencil_obj.users_collection:
        if coll not in original_collections:
            coll.objects.unlink(gpencil_obj)
    
    print(f"Converted curve '{name_c}' to grease pencil and maintained its original collections.")
